...
static bool crispasr_model_quantize(const std::string& fname_inp, const std::string& fname_out, ggml_ftype ftype) {
    ...
    // Higgs TTS: ConvTranspose1d weights (codec.ac_dec.block.*.conv_t1.*)
    // are 2D w_perm tensors that the DSP decoder treats as conv kernels.
    // Quantizing them adds audible artefacts in the upsampling path.
    // Keep them at original precision.
    const bool is_higgs = (arch.find("higgs") != std::string::npos);
    ...
    for (int i = 0; i < n_tensors; i++) {
        ...
        bool should_quantize =
            ...
            !(is_higgs && sname.find("codec.ac_dec.block.") != std::string::npos &&
              sname.find("conv_t1") != std::string::npos) &&
