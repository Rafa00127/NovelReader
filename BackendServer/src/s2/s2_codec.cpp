...
struct AudioCodec::Impl {
    ...
    std::unordered_map<std::string, int> convt_K;  // tensor name → K for pre-permuted conv_transpose weights
};
...
// Causal ConvTranspose1d via mul_mat + col2im_1d decomposition.
// Replaces ggml_conv_transpose_1d — mul_mat runs through cuBLAS/rocBLAS.
// trim_right: samples to crop from the right of raw output.
//   Typical causal trim: K - stride, yielding T_out = T_in * stride.
static ggml_tensor * convt1d_causal_decomp(ggml_context * ctx,
                                            ggml_tensor * x,        // [C, T] channels-first
                                            ggml_tensor * w_perm,   // [IC, K*OC] pre-permuted
                                            ggml_tensor * b,        // [OC] bias or nullptr
                                            int stride, int K, int trim_right) {
    const int OC = (int)w_perm->ne[1] / K;

    // mul_mat contracts IC → col: [K*OC, T_in]
    ggml_tensor * col = ggml_mul_mat(ctx, w_perm, x);

    // col2im: [K*OC, T_in] → [T_raw, OC]
    ggml_tensor * y = ggml_col2im_1d(ctx, col, stride, OC, 0);

    // Right-trim
    if (trim_right > 0) {
        y = ggml_view_2d(ctx, y, y->ne[0] - trim_right, y->ne[1], y->nb[1], 0);
        y = ggml_cont(ctx, y);
    }
...
static ggml_tensor * causal_conv_transpose_1d(ggml_context * ctx,
                                               ggml_tensor * weight, ggml_tensor * bias,
                                               ggml_tensor * x, int K, int stride, int crop_right) {
    // weight has been pre-permuted by pre_permute_convt_weights(): [IC, K*OC]
    int OC = (int)weight->ne[1] / K;
    ...
    return convt1d_causal_decomp(ctx, x, weight, bias, stride, K, crop_right);
}
...
static ggml_tensor * build_quantizer_stage_up(ggml_context * ctx, AudioCodec::Impl & impl,
    ...
    int K_up = impl.convt_K.at(prefix + ".0.conv.weight");
    x = causal_conv_transpose_1d(ctx, req(prefix + ".0.conv.weight"), req(prefix + ".0.conv.bias"), x, K_up, factor, 0);
...
bool AudioCodec::load_shared(SlowARModel* Model, gguf_context * shared_gguf_ctx, const std::string & gguf_path, int32_t gpu_device, BackendType backend_type) {
    ...
    // Always use GPU backend — no CPU fallback
...
#endif
    if (!impl_->backend) {
        std::cerr << "[Codec] GPU backend init failed." << std::endl;
        reset_codec_impl(*impl_);
        return false;
    }
...
static void permute_convt_weight_in_place(ggml_tensor * w, int K) {
    ...
    if (is_f32) {
        ggml_backend_tensor_get(w, host.data(), 0, n_elems * sizeof(float));
    } else {
        std::vector<ggml_fp16_t> src_f16(n_elems);
        ggml_backend_tensor_get(w, src_f16.data(), 0, n_elems * sizeof(ggml_fp16_t));
        for (size_t i = 0; i < n_elems; ++i)
            host[i] = ggml_fp16_to_fp32(src_f16[i]);
    }

    std::vector<float> perm(n_elems);
    for (int ic = 0; ic < IC; ++ic)
        for (int oc = 0; oc < OC; ++oc)
            for (int k = 0; k < K; ++k)
                perm[(oc * K + k) * IC + ic] = host[ic * OC * K + oc * K + k];

    w->ne[0] = IC;
    w->ne[1] = K * OC;
    w->ne[2] = 1;
    w->ne[3] = 1;
    w->nb[0] = is_f32 ? sizeof(float) : sizeof(ggml_fp16_t);
    w->nb[1] = (is_f32 ? sizeof(float) : sizeof(ggml_fp16_t)) * IC;
    w->nb[2] = (is_f32 ? sizeof(float) : sizeof(ggml_fp16_t)) * IC * (K * OC);
    w->nb[3] = (is_f32 ? sizeof(float) : sizeof(ggml_fp16_t)) * IC * (K * OC);

    if (is_f32) {
        ggml_backend_tensor_set(w, perm.data(), 0, n_elems * sizeof(float));
    } else {
        std::vector<ggml_fp16_t> perm_f16(n_elems);
        for (size_t i = 0; i < n_elems; ++i)
            perm_f16[i] = ggml_fp32_to_fp16(perm[i]);
        ggml_backend_tensor_set(w, perm_f16.data(), 0, n_elems * sizeof(ggml_fp16_t));
    }
}

void AudioCodec::pre_permute_convt_weights() {
    if (!impl_ || !impl_->ctx_w) return;

    const std::string & tp = impl_->tprefix;

    // Quantizer upsample stages
    const size_t n_up = impl_->quantizer_downsample_factor.size();
    for (size_t i = 0; i < n_up; ++i) {
        std::string name = tp + "quantizer.upsample." + std::to_string(i) + ".0.conv.weight";
        ggml_tensor * w = ggml_get_tensor(impl_->ctx_w, name.c_str());
        if (!w || w->ne[2] <= 1) continue;  // already 2D (pre-permuted)
        int K = (int)w->ne[0];
        impl_->convt_K[name] = K;
        permute_convt_weight_in_place(w, K);
    }

    // Decoder model blocks
    const int n_dec = static_cast<int>(impl_->decoder_rates.size());
    for (int i = 0; i < n_dec; ++i) {
        std::string name = tp + "decoder.model." + std::to_string(i + 1) + ".block.1.conv.weight";
        ggml_tensor * w = ggml_get_tensor(impl_->ctx_w, name.c_str());
        if (!w || w->ne[2] <= 1) continue;  // already 2D (pre-permuted)
        int K = (int)w->ne[0];
        impl_->convt_K[name] = K;
